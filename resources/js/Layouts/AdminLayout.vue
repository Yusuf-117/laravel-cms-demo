        ...
        <div class="flex">
            <aside class="w-64 shrink-0 border-r border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
                <div class="h-14 flex items-center px-4 border-b border-zinc-200 dark:border-zinc-800">
                    <Link
                        :href="route('admin.dashboard')" class="font-semibold">
                        Admin
                    </Link>
                </div>
            ...
            <div class="flex-1 min-w-0">
                <header class="h-14 bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4">
                    ...
                    <div class="flex items-center gap-3">
                        <button
                            type="button"
                            @click="toggleTheme" class="text-sm rounded px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800">
                            {{ theme === "dark" ? "Light" : "Dark" }}
                        </button>

                        <div
                            v-if="user" class="text-sm text-zinc-600 dark:text-zinc-400">
                        ...
                        <button
                            v-if="user"
                            type="button"
                            @click="logout" class="text-sm rounded px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800">
                            Logout
                        </button>

                        <Link
                            v-else
                            :href="route('login')" class="text-sm rounded px-3 py-2 hover:bg-zinc-100 dark:hover:bg-zinc-800">
                            Login
                        </Link>
                    </div>
                </header>
