...
const initialContent = computed(() => {
    return props.modelValue?.trim() ? props.modelValue : "<p></p>";
});
...
function openMedia() {
    ...
    fetch(route("media.list"))
        .then((res) => res.json())
        .then((data) => {
            media.value = data.media.filter((m) =>
...
<template>
    <div class="border rounded-xl overflow-hidden bg-white dark:bg-zinc-900 dark:border-zinc-700">
        <!-- HEADER -->
        <div
            class="flex justify-between items-center px-3 py-2 border-b dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800">
